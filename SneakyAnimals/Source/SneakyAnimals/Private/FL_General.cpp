...
bool UFL_General::Save_UserProfile_with_idx(int32 uniqueIdx, const FStructure_UserProfile& S_UserData)
{
	// Save Game 생성
	USG_UserProfile* SG_UserProfile_inst = Cast<USG_UserProfile>(UGameplayStatics::CreateSaveGameObject(USG_UserProfile::StaticClass()));
	...
	// 저장 슬롯에 데이터를 저장
	return UGameplayStatics::SaveGameToSlot(SG_UserProfile_inst, TEXT("UserProfile_Slot"), uniqueIdx);

}

FUserProfileResult UFL_General::Get_UserProfile_with_idx(int32 uniqueIdx)
{

	FUserProfileResult result;

	if (UGameplayStatics::DoesSaveGameExist(TEXT("UserProfile_Slot"), uniqueIdx))
	{
		// UserProfile_Slot 이 있으면 S_UserProfile과 bool success = true 리턴하기
		USG_UserProfile* sg = Cast<USG_UserProfile>(UGameplayStatics::LoadGameFromSlot(TEXT("UserProfile_Slot"), uniqueIdx));
		if (sg)
		{
			result.S_UserProfile = sg->S_UserProfile;
			result.success = true;
